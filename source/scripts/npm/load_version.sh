#!/bin/bash
###------------------------------------------------------------------------------------------------
# alias:   cid.npm.load_version
# script:  load_version.sh
# purpose: Loads Package Version from specified package.json file
# version: 1.0.0
###------------------------------------------------------------------------------------------------

###------------------------------------------------------------------------------------------------
## Config
declare -r SELF_IDENTITY="npm_load_version"
declare -r SELF_IDENTITY_H="NPM (Load Version from package.json)"
...
TMP_VALUE="$(cat ${FILE_TARGET} | grep '"version"' | head -1 | awk -F: '{ print $2 }' | sed 's/[",]//g' | tr -d '[:space:]')"
if(! is_empty "${TMP_VALUE}"); then
    log "- Package Version:   [${TMP_VALUE}]"
